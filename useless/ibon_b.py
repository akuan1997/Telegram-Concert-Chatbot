    ...
    with sync_playwright() as p:
        ...
        while True:
            try:
                ...
                page_ibon.wait_for_timeout(1500)

                events = page_ibon.query_selector_all("#content > div > div > app-activity-for-index > div:nth-child(1) > owl-carousel-o > div > div.owl-stage-outer.ng-star-inserted > owl-stage > div > div > div")
                ...
                for i in range(last_finished_index + 1, len(events)):
                    ...
                    if page_ibon.locator("#detall_info_1 > div").is_visible():
                        # 下載內文
                        inner_text = page_ibon.locator("#detall_info_1 > div").inner_text()
                        ...
                        page_ibon.wait_for_timeout(1500)
                        events = page_ibon.query_selector_all(
                            "#content > div > div > app-activity-for-index > div:nth-child(1) > owl-carousel-o > div > div.owl-stage-outer.ng-star-inserted > owl-stage > div > div > div")
                        page_ibon.wait_for_timeout(1500)
                    # 無法讀取到內文
                    else:
                        print('格式怪怪的 跳過')
