...
#     seg_list = jieba.cut(data[i]['int'], cut_all=True)
#     print(seg_list)

# for i in range(len(data)):
#     if data[i]['tit'] == '浮現祭EMERGE FEST 2024':
#         print(i)

# print(data[0]['int'])
# seg_list = jieba.cut(data[137]['int'], cut_all=True)
# seg_list = jieba.cut(data[137]['int'], cut_all=False)
# print("【全模式】：" + "/ ".join(seg_list)) # 字串
seg_list = jieba.cut_for_search(data[137]['int'])
print("【返回列表】：{0}".format(seg_list)) # 列表
...
#
# # 創建TF-IDF向量器，指定中文分詞函數 jieba
# vectorizer = TfidfVectorizer(tokenizer=chinese_tokenizer, min_df=2, max_df=0.5, ngram_range=(1, 2))
#
# # 轉換文檔為TF-IDF表示
