...
from functions.get_concert_new_old import *
from functions.get_data_from_text import *
from functions.concert_translation import *
from functions.get_enews_emails import *
from functions.concert_send_email import *
...
with open('../concert_data_json/concert_zh.json', 'r', encoding='utf-8') as f:
...
def get_new_old(json_filename):
    ...
    # 寫進json裡面
    with open('../concert_data_json/concert_zh.json', "w", encoding="utf-8") as f:
        json.dump(all_data, f, indent=4, ensure_ascii=False)
        print('寫入成功')

    # json_in_order(json_filename)
    json_in_order("../concert_data_json/concert_zh.json")
...
def kktix_get_title_str(page, title):
    try:
        page.wait_for_selector("#registrationsNewApp > div > div:nth-child(2) > div.narrow-wrapper.ng-scope > div > h1",
                               timeout=5000)
        # 標題 str
        if page.locator(
...
def email_content():
    ...
    if not (check_if_today(new_file) or check_if_today(plus_file)):
        ...
        formatted_str_list.append("The is no information today.")
