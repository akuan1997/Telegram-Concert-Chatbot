...
# 正在等待爬蟲的category網頁
not_executed_categories = 'artists_categories_waiting.txt'
# 要寫入哪一個檔案
write_json_file = 'artists3.json'
...
with sync_playwright() as p:
    ...
    #     page = context.new_page()
    #
    #     with open('artists_categories_waiting.txt', 'r', encoding='utf-8') as f:
    #         waiting_categories = f.readlines()
    #     waiting_categories = [waiting_category.replace('\n', '') for waiting_category in waiting_categories]
    #
    #     for waiting_category in waiting_categories:
    #         page.goto(waiting_category)
    #         page.wait_for_load_state('load')
    ...
    #
    #         if page.url not in processed_categories:
    #             country_singers_no_sub_category(page, "artists_singers_bands.txt", "artists3.json")
    #         else:
    #             print('這個頁面已經完成了!')
    #
    #         with open('artists_categories.txt', 'a', encoding='utf-8') as f:
