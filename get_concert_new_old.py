            ...
            if new_data[i]['pin'] == old_data[j]['pin']:
                ...
                if new_data[i]['sdt'] != old_data[j]['sdt']:
                    ...
                    if len(new_data[i]['sdt']) > len(old_data[j]['sdt']):
                        ...
                        if revise:
                            print(
                                f"相同數量的sdt / {old_data[j]['sdt']} -> {new_data[i]['sdt']} / 修改all_data\n{new_data[i]['pin']}\n---")
                            all_data[pin_index_in_all_data]['sdt'] = new_data[i]['sdt']
                        else:
                            print(
                                f"相同數量的sdt，但是出現空白 / {old_data[j]['sdt']} -> {new_data[i]['sdt']} / 不修改all_data\n{new_data[i]['pin']}\n---")
                    else:
                        print(f"{new_data[i]['tit']}")
                        print(
                ...
                if new_data[i]['pdt'] != old_data[j]['pdt']:
                    ...
                    if len(new_data[i]['pdt']) > len(old_data[j]['pdt']):
                        ...
                        if revise:
                            print(
                                f"相同數量的pdt / {old_data[j]['pdt']} -> {new_data[i]['pdt']} / 修改all_data\n{new_data[i]['pin']}\n---")
                            all_data[pin_index_in_all_data]['pdt'] = new_data[i]['pdt']
                        else:
                            print(
                                f"相同數量的pdt，但是出現空白 / {old_data[j]['pdt']} -> {new_data[i]['pdt']} / 不修改all_data\n{new_data[i]['pin']}\n---")
                    else:
                        print(f"{new_data[i]['tit']}")
                        print(
                            f"pdt減少 / {old_data[j]['pdt']} -> {new_data[i]['pdt']} / 不修改all_data\n{new_data[i]['pin']}\n---")
                ...
                if new_data[i]['loc'] != old_data[j]['loc']:
                    ...
                    if len(new_data[i]['loc']) > len(old_data[j]['loc']):
                        ...
                        if revise:
                            print(
                                f"相同數量的loc / {old_data[j]['loc']} -> {new_data[i]['loc']} / 修改all_data\n{new_data[i]['pin']}\n---")
                            all_data[pin_index_in_all_data]['loc'] = new_data[i]['loc']
                        else:
                            print(
                                f"相同數量的loc，但是出現空白 / {old_data[j]['loc']} -> {new_data[i]['loc']} / 不修改all_data\n{new_data[i]['pin']}\n---")
                    else:
                        print(f"{new_data[i]['tit']}")
                        print(
                            f"loc減少 / {old_data[j]['loc']} -> {new_data[i]['loc']} / 不修改all_data\n{new_data[i]['pin']}\n---")
...
def get_new_delete_compare_concerts(new_but_old_pins, old_but_new_pins, new_data, old_data, all_data):
    ...
    all_data = new_concerts(new_but_old_pins, new_data, all_data)
    # 2. 移除那些演唱會無法獲得的演唱會資訊
    # show_old_but_new(old_but_new_pins, old_data)
    all_data = old_concert_delete(old_but_new_pins, all_data)
    print('---')
    # 3. 比較內文
